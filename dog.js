...
camera.position.set(1, 2, 3); // Move camera closer to the model
...
let jumpHeight = 0.25; // Maximum jump height
...
function animate() {
  ...
  // If the dog model is loaded and turning, animate the rotation
  if (dogModel) {
    if (isTurning) {
      const rotationStep = turnSpeed * turnDirection; // Calculate the rotation for this frame
      dogModel.rotation.y += rotationStep; // Rotate the model around the y-axis
      ...
      if (turnAmount >= 2 * Math.PI) {
        ...
        dogModel.rotation.y = 0; // Reset to the original rotation if needed
      }
    }

    // If the dog is jumping, animate the jump
    if (isJumping) {
      if (jumpDirection === 1) {
        // Moving up
        dogModel.position.y += jumpSpeed;
        if (dogModel.position.y >= 1.05 + jumpHeight) { // If reached the max height
          jumpDirection = 0; // Start moving down
        }
      } else {
        // Moving down
        dogModel.position.y -= jumpSpeed;
        if (dogModel.position.y <= 1.05) { // If back to original position
          dogModel.position.y = 1.05; // Snap to original height
