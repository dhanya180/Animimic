...
camera.position.set(0, 0, 0); // Move camera closer to the model
...
let jumpHeight = 0.025; // Maximum jump height
let jumpSpeed = 0.001; // Speed of the jump
...
audioLoader.load('audio/meow.mp3', (buffer) => {
  ...
  sound.setLoop(0); // Play only once
...
function animate() {
  ...
  // If the cat model is loaded and turning, animate the rotation
  if (catModel) {
    if (isTurning) {
      const rotationStep = turnSpeed * turnDirection; // Calculate the rotation for this frame
      catModel.rotation.y += rotationStep; // Rotate the model around the y-axis
      ...
      if (turnAmount >= 2 * Math.PI) {
        ...
        catModel.rotation.y = 0; // Reset to the original rotation if needed
      }
    }

    // If the cat is jumping, animate the jump
    if (isJumping) {
      if (jumpDirection === 1) {
        // Moving up
        catModel.position.y += jumpSpeed;
        if (catModel.position.y >= 1.05 + jumpHeight) { // If reached the max height
          jumpDirection = 0; // Start moving down
        }
      } else {
        // Moving down
        catModel.position.y -= jumpSpeed;
        if (catModel.position.y <= 1.05) { // If back to original position
            catModel.position.y = 1.05; // Snap to original height
